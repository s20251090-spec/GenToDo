...
export default function AIChatModule({ onBack, storage, saveStorage }: { onBack: () => void; storage: any; saveStorage: (next: any) => void }) {
  ...
  return <div className="h-full flex flex-col bg-gradient-to-b from-[#F7FAFF] to-white text-[#1D2129]">
    <style>{`::-webkit-scrollbar{width:4px}::-webkit-scrollbar-thumb{background:#CBD5E1;border-radius:10px}`}</style>
    <header className="w-full px-4 py-3 flex items-center justify-between border-b border-gray-100/70 bg-white/85 backdrop-blur z-30">
      ...
      <div className="flex items-center gap-2">
        ...
        <button onClick={() => setMessages([])} className="px-3 py-1.5 rounded-[40px] bg-[#E8F3FF] text-[#165DFF] text-sm font-medium">新对话</button>
      </div>
    </header>
    <main className="flex-1 overflow-y-auto px-4 py-4">
      {messages.length===0 ? <div className="h-full flex flex-col items-center justify-center text-center gap-4">
        <h2 className="text-2xl font-bold">你好！今天想一起完成什么学习目标？</h2>
      </div> : <div className="flex flex-col gap-3">{messages.map((m,i)=><div key={i} className={`max-w-[85%] px-4 py-3 rounded-2xl ${m.role==='user'?'self-end bg-[#165DFF] text-white':'self-start bg-white border border-blue-50 shadow-sm'}`}>{m.content}</div>)}</div>}
    </main>
    <footer className="w-full px-4 py-3 border-t border-gray-100 bg-white/90 backdrop-blur z-20">
      <div className="w-full flex items-center gap-2 bg-gray-50 rounded-[24px] px-2 py-2 border border-gray-200">
        ...
        <button onClick={send} disabled={isLoading} className="w-12 h-12 rounded-full bg-[#165DFF] text-white disabled:opacity-50 flex items-center justify-center"><SendHorizonal size={16} /></button>
      </div>
    ...
    <nav className="w-full px-6 py-3 border-t border-gray-100 bg-white flex items-center justify-between z-30">
      <button onClick={onBack} className="text-[#165DFF] text-xs">首页</button><button onClick={() => setShowModal('master')} className="text-xs text-gray-400">计划</button><button className="text-xs text-gray-400">历史</button><button className="text-xs text-gray-400">我的</button>
    </nav>
    ...
    />
    {showModal && <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4"><div className="bg-white rounded-[32px] p-6 w-full max-w-lg"><h3 className="font-bold mb-3">{showModal==='master'?'总体学习计划':showModal==='daily'?'每日学习计划':'修改学习计划'}</h3><textarea value={modalInput} onChange={(e) => setModalInput(e.target.value)} className="w-full border rounded-2xl p-3 min-h-[120px]" placeholder="请输入你的具体要求..."/><div className="mt-3 flex gap-2 justify-end"><button onClick={()=>setShowModal(null)} className="px-4 py-2 rounded-[40px] border">取消</button><button onClick={handleGeneratePlan} className="px-4 py-2 rounded-[40px] bg-[#165DFF] text-white">生成计划</button></div></div></div>}
  </div>;
}
